...
from federated_learning.datasets.data_distribution import mixture_distribution_split_noniid
from federated_learning.datasets.data_distribution import distribute_dataset
...
from federated_learning.utils import convert_numpy
...
def train_subset_of_clients(epoch, args, clients, poisoned_workers):
    ...
    """
    kwargs = args.get_round_worker_selection_strategy_kwargs()#keyword arguments
    kwargs["current_epoch_number"] = epoch

    random_workers = args.get_round_worker_selection_strategy().select_round_workers(#random定义
    ...
    new_nn_params = average_nn_parameters(parameters,poisoned_workers)
...
def run_exp(replacement_method, num_poisoned_workers, KWARGS, client_selection_strategy, idx):
    ...
    # Distribute batches nonIID
    #distributed_train_dataset =mixture_distribution_split_noniid(train_data_loader,10, args.get_num_workers(),-1,1)
    ...
    distributed_train_dataset = poison_data(logger, distributed_train_dataset, args.get_num_workers(), poisoned_workers, replacement_method,idx)
