...
def convert_distributed_gandata_into_numpy(distributed_dataset):
    """
    ...
    for worker_idx in range(len(distributed_dataset)):
        ...
        X_ = numpy.array([tensor.cpu().numpy() for batch in worker_training_data for tensor in batch[0]])
        Y_ = numpy.array([tensor.cpu().numpy() for batch in worker_training_data for tensor in batch[1]])
