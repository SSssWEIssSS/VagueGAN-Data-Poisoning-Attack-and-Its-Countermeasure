...
SAVE_NAME = "pca_defense_results.jpg"
SAVE_SIZE = (18, 14)
...
def plot_gradients_3d(gradients,SAVE_NAME):
    ...
    for (worker_id, gradient) in gradients:
        if worker_id in POISONED_WORKER_IDS:
            ax.scatter(gradient[0], gradient[1], gradient[2], alpha = 1, color="blue", marker="x", s=500)#, linewidth=5
        else:
            ax.scatter(gradient[0], gradient[1], gradient[2], alpha = 0.5,color="orange", s=180)
...
if __name__ == '__main__':
    ...
    plot_gradients_2d(zip(worker_ids, dim_reduced_gradients),SAVE_NAME)
